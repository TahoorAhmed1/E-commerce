...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.tittle in shoes) {
      if (!shoes[item.tittle].color.includes(item.color) && item.avalibalQuantity > 0) {
        shoes[item.tittle].color.push(item.color)
      }
      if (!shoes[item.tittle].size.includes(item.size) && item.avalibalQuantity > 0) {
        shoes[item.tittle].size.push(item.size)
      }
    }
    else {
      shoes[item.tittle] = JSON.parse(JSON.stringify(item))
      if (item.avalibalQuantity > 0) {
        shoes[item.tittle].color = [item.color]
        shoes[item.tittle].size = [item.size]
