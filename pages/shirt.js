        ...
        <div className="container px-5 py-24 mx-auto">
          ...
          {Object.keys(products).length == 0 && <p>This product is out of stock . New product avalibal soon </p> }
            {Object.keys(products).map((item) => {
              return <div key={Math.random()*1} className="lg:w-1/5 md:w-1/2 p-4 w-full m-5 shadow-lg cursor-pointer">
                ...
                <Link   passHref={true} href={`/product/${products[item].slug}`}>
                  <div key={Math.random()*1} className="mt-4 text-center md:text-left ">
...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.tittle in shirts) {
      if (!shirts[item.tittle].color.includes(item.color) && item.avalibalQuantity > 0) {
        shirts[item.tittle].color.push(item.color)
      }
      if (!shirts[item.tittle].size.includes(item.size) && item.avalibalQuantity > 0) {
        shirts[item.tittle].size.push(item.size)
      }
    } 
    else {
      shirts[item.tittle] = JSON.parse(JSON.stringify(item))
      if (item.avalibalQuantity > 0) {
        shirts[item.tittle].color = [item.color]
        shirts[item.tittle].size = [item.size]
